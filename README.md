...
`- Week3/
    |- OOPbuoi3.py        - Create Point class, calculate distance between 2 points and to the origin (0,0)
                          - Gao Ranger class, manage team list, attributes (weapons, colors), and display
                          - Dog kennel simulation: interact (poke, feed, pet) and update emotional states
                          - Car driving simulation: adjust speed, handle crash decisions
                          - Bank account management: deposit, withdraw, and check balance
                          - 2D Geometry (Think Python Chap 15): Point, Circle, Rectangle classes
                          - Check if a point or rectangle is completely inside a circle
                          - Check intersection/overlap between a circle and a rectangle
                          - Employee class with private attributes (Encapsulation) and static constants
